...
shortString = """Lorm ipsum horzum flank full board filly and pace points of a horse performance class pastern breeching or crop near side. Pointing bronc. Bronc dope, doping yearling quirt point coloration and pinhooking or pinhooking. Dogger girth or bit pastern gooseneck four-in-hand. Paddock or quarter horse claiming race. Pinhooking and whinney. Hunt seat poor doer or equine amble amble and cinch sour. Equitation from and ai. Semi-feral. Bronc. Ankle cold-backed or dope, doping outlaw. Bolting and bronc place and unshod or twitch wheeler yearling eventing on the bit. Win get combined driving. Galloway and hand gallop and bronc australian stock saddle three-quarter genetic brother/sister; or sport horse or equitation western riding. Tack room. Jump, to jump or quarter horse or crossbred and hand gallop billet or quarter horse trotting races or yearling knacker or branding and near side galloway or cluster mare. Stable stride parrot mouth sport horse. Natural cover, live cover or wild horse or coupling. Yearling horse blanket or breed registry and knee and pace full-brother furlong saddle smooth mouth rearing. Allowance race balk. Crowhop or"""

"""Chapter 1 of Adventures of Huckleberry Finn by Mark Twain, from http://www.gutenberg.org/cache/epub/76/pg76.txt"""
longString = """CHAPTER I.
...
# timeit.timeit('ft.base64(shortString)', 'import ft')


start = time.time()

for i in rng:
	ft.base64(shortString)

end = time.time()

short_normal_ft = end - start
...
for i in rng:
	base64.b64encode(shortString)

end = time.time()

short_normal_b64 = end - start
...
for i in rng:
	ft.base64(longString)

end = time.time()

long_normal_ft = end - start


start = time.time()

for i in rng:
	base64.b64encode(longString)

end = time.time()

long_normal_b64 = end - start


print "Normal base64, short string %i iterations" % iterations
print "Builtin: %f" %  short_normal_b64
print "FT: %f" % short_normal_ft
print "FT vs BI %fs runtime, %fx faster" % ((short_normal_ft - short_normal_b64), (short_normal_b64/short_normal_ft))
print ""
print ""

print "Normal base64, long string %i iterations" % iterations
print "Builtin: %f" %  long_normal_b64
print "FT: %f" % long_normal_ft
print "FT vs BI %fs runtime, %fx faster" % ((long_normal_ft - long_normal_b64), (long_normal_b64/long_normal_ft))
print ""
print ""
...
for i in rng:
	ft.base64string(shortString)

end = time.time()

short_str_ft = end - start


start = time.time()

for i in rng:
	base64.encodestring(shortString)

end = time.time()

short_str_b64 = end - start


start = time.time()

for i in rng:
	ft.base64string(longString)

end = time.time()

long_str_ft = end - start
...
long_str_b64 = end - start

print "String base64, short string %i iterations" % iterations
print "Builtin: %f" %  short_str_b64
print "FT: %f" % short_str_ft
print "FT vs BI %fs runtime, %fx faster" % ((short_str_ft - short_str_b64), (short_str_b64/short_str_ft))
print ""
print ""

print "String base64, long string %i iterations" % iterations
print "Builtin: %f" %  long_str_b64
print "FT: %f" % long_str_ft
print "FT vs BI %fs runtime, %fx faster" % ((long_str_ft - long_str_b64), (long_str_b64/long_str_ft))
